...
class GeometryExpression  
  ...
  def eval_prog env
    self  # do nothing for super class
  end
...
class Point < GeometryValue
  ...
  # shape s and the seg.  seg is an instance of LineSegment
  def intersectWithSegmentAsLineResult seg
    if (seg.x1 - GeometryExpression::Epsilon <= @x &&
        @x <= seg.x2 + GeometryExpression::Epsilon) ||
      (seg.x2 - GeometryExpression::Epsilon <= @x &&
        @x <= seg.x1 + GeometryExpression::Epsilon)
      self
    else
      NoPoints.new()
    end
...
class LineSegment < GeometryValue
  ...
  def intersectWithSegmentAsLineResult seg
    if real_close(seg.x1, seg.x2)   # vertical line
      a, b = seg.y1 > @y1 ? [self, seg] : [seg, self]
      ...
      a, b = seg.x1 > @x1 ? [self, seg] : [seg, self]
...
class Intersect < GeometryExpression
  ...
  def preprocess_prog
    if @e1 == @e1.preprocess_prog && @e2 == @e2.preprocess_prog
      self
    else
